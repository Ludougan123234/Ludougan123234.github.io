...
## 2.1 Observing the data set
Before we really enter the analysis, let's take a look at the data that we will be using in this demonstration. Normally, it would be hard to look through the entire data set since there are so many entries in one data set (in the sat.act data set, there are 700 entries). So, to look at the first few rows of the data, use the `head()` function. 
...
## 2.2 Descriptive statistics analysis and terminologies
To do a descriptive statistics analysis, it is as simple as using only 1 line of code, using the `describe()` function from the `psych` package. See the following example: 
``` {r class.source = "code1"}
...
And there you have the descriptive statistics for your data set!

## 2.3 Deciphering the output 
...
|range| = max - min|
|skew| Stands for "skewness" <br>If your data is <strong>positively skewed</strong>, there is more data on the right tail. <br>If your data is <strong>negatively skewed</strong>, there is more data on the left tail of the distribution. ![Skewness](skew.jpeg)|
...
# Syntax: describeBy(data, data$column_name)
...
<li><strong>Formatting your report</strong>: The MLA 8th edition does not seem to give any guidance on reporting statistical results. Thus, you might want to consider using APA to report your results. For more guidance on reporting statistical results, see [this page](https://www.scribbr.com/apa-style/numbers-and-statistics/)</li>
</ul>


That wraps up our discussion on descriptive statistics. You can choose to: <br>
[Go back to the main page](https://ludougan123234.github.io/Index.html) or <br>
...
5. Kotu, V., & Deshpande, B. (2019). Chapter 3 - Data Exploration. In V. Kotu & B. Deshpande (Eds.), Data Science (Second Edition) (pp. 39-64). Morgan Kaufmann. https://doi.org/https://doi.org/10.1016/B978-0-12-814761-0.00003-4 
